...
export default function WorkspaceTabs({ activeTab, onChange }: WorkspaceTabsProps) {
  return (
    <div className="flex flex-wrap border border-slate-300 bg-white rounded overflow-hidden" id="workspace-tabs-navigator">
      {TABS.map(({ id, label, icon: Icon }, index) => (
        <button
          key={id}
          onClick={() => onChange(id)}
          aria-label={label}
          className={`min-w-0 flex-1 basis-1/2 py-3 text-center transition duration-150 flex items-center justify-center gap-2 text-xs font-bold uppercase tracking-wider sm:basis-0 ${index < TABS.length - 1 ? 'border-r border-slate-300' : ''} ${
            activeTab === id
